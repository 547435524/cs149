...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  int remainWidth = N % VECTOR_WIDTH;
  __cs149_mask maskRemain = _cs149_init_ones(remainWidth);
  __cs149_mask maskAll = _cs149_init_ones(VECTOR_WIDTH);
  __cs149_vec_float vecX;
  __cs149_vec_float vecOut;
  __cs149_vec_float vecConstOut;
  _cs149_vset_float(vecOut, 1.f, maskAll);
  _cs149_vset_float(vecConstOut, 9.999999f, maskAll);


  __cs149_vec_int vecY;
  __cs149_vec_int vecIntZero = _cs149_vset_int(0); 
  __cs149_vec_int vecIntOne = _cs149_vset_int(1); 
  __cs149_mask maskGtZero;
  for (int i = 0; i< N; i += VECTOR_WIDTH) {
    _cs149_vload_float(vecX, values+i, maskAll);               // x = values[i];
    _cs149_vload_int(vecY, exponents+i, maskAll);  
    
    _cs149_vgt_int(maskGtZero, vecY, vecIntZero, maskAll);
    // __cs149_mask maskLtZero = _cs149_mask_not(maskGtZero);
    // _cs149_vset_float(vecOut, 1.f, maskLtZero);
    _cs149_vset_float(vecOut, 1.f, maskAll);

    while(_cs149_cntbits(maskGtZero)){
      _cs149_vmult_float(vecOut, vecOut, vecX, maskGtZero);
      _cs149_vsub_int(vecY, vecY, vecIntOne, maskGtZero);
      _cs149_vgt_int(maskGtZero, vecY, vecIntZero, maskAll);
    }
    __cs149_mask vecResultMask;
    _cs149_vgt_float(vecResultMask, vecOut, vecConstOut, maskAll);
    _cs149_vset_float(vecOut, 9.999999f, vecResultMask);
    _cs149_vstore_float(output+i, vecOut, maskAll);
  }

  //deal with remain
  if(remainWidth != 0){
    _cs149_vload_float(vecX, values + N - remainWidth, maskRemain);               // x = values[i];
    _cs149_vload_int(vecY, exponents + N - remainWidth, maskRemain);  
    
    _cs149_vgt_int(maskGtZero, vecY, vecIntZero, maskRemain);
    // __cs149_mask maskLtZero = _cs149_mask_not(maskGtZero);
    // _cs149_vset_float(vecOut, 1.f, maskLtZero);
    _cs149_vset_float(vecOut, 1.f, maskRemain);
    ...
    __cs149_mask vecResultMask;
    _cs149_vgt_float(vecResultMask, vecOut, vecConstOut, maskRemain);
    _cs149_vset_float(vecOut, 9.999999f, vecResultMask);
    _cs149_vstore_float(output+ N - remainWidth, vecOut, maskRemain);
  }
}
...
float arraySumVector(float* values, int N) {
  ...
  //
  __cs149_vec_float vecTemp;
  __cs149_mask maskAll = _cs149_init_ones(VECTOR_WIDTH);
  __cs149_vec_float vecSum ;
  float sum = 0.0;
  _cs149_vset_float(vecSum, 0.f, maskAll);

  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    _cs149_vload_float(vecTemp, values+i, maskAll);               // x = values[i];

    _cs149_vadd_float(vecSum, vecSum, vecTemp, maskAll);
