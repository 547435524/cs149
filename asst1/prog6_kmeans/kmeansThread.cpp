...
void computeAssignments(WorkerArgs *const args) {
  // 使用vector避免手动内存管理
  std::vector<double> minDist(args->M, 1e30);

  // 初始化clusterAssignments
  for (int m = 0; m < args->M; m++) {
    args->clusterAssignments[m] = -1;
  }

  // lambda函数用于线程执行
  auto assignWork = [&](int start, int end) {
  ...
  int chunkSize = args->M / numThreads;  // 数据点的数量

  // 创建并启动线程
  for (int t = 0; t < numThreads; ++t) {
    int threadStart = t * chunkSize;
    int threadEnd = (t == numThreads - 1) ? args->M : threadStart + chunkSize;
    printf("thread start: %d, thread end: %d\n", threadStart, threadEnd);
    
    threads.emplace_back(assignWork, threadStart, threadEnd);
  }
...
void computeAssignmentsBack(WorkerArgs *const args) {
